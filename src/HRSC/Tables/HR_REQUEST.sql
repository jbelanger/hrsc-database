CREATE TABLE [HRSC].[HR_REQUEST] (
    ...
    [RECRUITMENT_TYPE_ID]        BIGINT          NULL,
    ...
    CONSTRAINT [FK_HR_REQUEST_CD_RECRUITMENT_TYPE_RECRUITMENT_TYPE_ID] FOREIGN KEY ([RECRUITMENT_TYPE_ID]) REFERENCES [HRSC].[CD_RECRUITMENT_TYPE] ([RECRUITMENT_TYPE_ID]),
...
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_UK_SARG]
    ON [HRSC].[HR_REQUEST]([HR_REQUEST_ID] ASC, [MODE_OF_RECEIPT_ID] ASC, [HR_REQUEST_IDENTIFIER] ASC) WITH (FILLFACTOR = 95);


GO
CREATE NONCLUSTERED INDEX [HRRQST_ISPRTCTD]
    ON [HRSC].[HR_REQUEST]([IS_PROTECTED] ASC)
    INCLUDE([HR_REQUEST_ID]) WITH (FILLFACTOR = 95);
