...
CREATE PROCEDURE [HRSC_APP].[usp_REQUEST_Update_New]
...
@pRegionID bigint, @pRcCode nvarchar(10), @pOrganizationID bigint, @pSubOrganizationID bigint, 
...
@pEffectiveDate date, @PayPeriodNumber bigint, @ProcessingOpenDate date, @ProcessingCloseDate date,  
@pReturnCode bigint OUTPUT,@pXmlManagerSD XML, @pXmlManagerHost XML, @pXmlManagerSubmitter XML = NULL, @pXmlAdministrative XML = NULL,  @pXmlSupervisor XML = NULL
WITH EXEC AS CALLER
AS
------------------------------------------------------
-- REQUEST_Update
...
-- Insert HostManager			 
      ...
      BEGIN
-- Insert Submitter			 
		  EXEC @ReturnCall = usp_EMPLOYEE_ROLE_FOR_REQ_InsertForRequestID @pRequestID, @pUser, @pXmlManagerSubmitter
		  IF @ReturnCall != 0
			 SET @pReturnCode = @ReturnCall
		  ELSE
			 SET @pReturnCode = 0			 
...
-- Insert Administrative		 
		  EXEC @ReturnCall = usp_EMPLOYEE_ROLE_FOR_REQ_InsertForRequestID @pRequestID, @pUser, @pXmlAdministrative
		  IF @ReturnCall != 0
			 SET @pReturnCode = @ReturnCall
		  ELSE
			 SET @pReturnCode = 0			 
	  ...
	  IF @ReturnCall = 0       
		   BEGIN
-- Insert Supervisor		 
		  EXEC @ReturnCall = usp_EMPLOYEE_ROLE_FOR_REQ_InsertForRequestID @pRequestID, @pUser, @pXmlSupervisor
		  IF @ReturnCall != 0
			 SET @pReturnCode = @ReturnCall
		  ELSE
			 SET @pReturnCode = 0			 
