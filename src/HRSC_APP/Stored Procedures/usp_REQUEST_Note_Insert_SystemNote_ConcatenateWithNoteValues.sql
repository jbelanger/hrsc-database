...
CREATE   PROCEDURE [HRSC_APP].[usp_REQUEST_Note_Insert_SystemNote_ConcatenateWithNoteValues]
	@lngReqId bigint, 
	@lngIntId bigint, 
...
AS
------------------------------------------------------
-- usp_REQUEST_Note_Insert_SystemNote_ConcatenateWithNoteValues
-- Insert a new system generated Note in REQUEST_NOTE with values taken from CD_Note_Type and then concatenated with sent in parameters
-- Yves Robichaud 2015/06/25
------------------------------------------------------

DECLARE @Error int;
--DECLARE @lngReqId bigint;
DECLARE @NoteTypeID bigint;
DECLARE @strNoteFrTemp nvarchar(500);
DECLARE @strNoteEnTemp nvarchar(500);
...
SELECT
		...
		@strNoteFrTemp = RTRIM([NOTE_TYPE_NAME_DESC_FR]),
		@strNoteEnTemp = RTRIM([NOTE_TYPE_NAME_DESC_EN])
FROM [HRSC].[CD_NOTE_TYPE]
WHERE [HRSC].[CD_NOTE_TYPE].[NOTE_TYPE_CODE] = @strNoteTypeCode;

DECLARE @strNote nvarchar(500) = @strNoteEnTemp + @strNoteEn + ' / ' + @strNoteFrTemp + @strNoteFr
...
( @lngReqId,
  ...
  @strNote,
  @NoteTypeID,
  @strNoteEnTemp + @strNoteEn,
  @strNoteFrTemp + @strNoteFr,
  @strUserCreated,
  GetDate());
