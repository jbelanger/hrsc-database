...
CREATE PROCEDURE [HRSC_APP].[usp_REQUEST_SearchByPRI_All]
...
FROM	(SELECT		REQ.HR_REQUEST_ID
		 FROM		HRSC.HR_REQUEST REQ
					...
					--INNER JOIN HRSC.HR_REQUEST_STATUS STAT ON REQ.HR_REQUEST_ID = STAT.HR_REQUEST_ID
		 ...
		 WHERE		--STAT.HR_REQUEST_STATUS_ID = (select MAX(HR_REQUEST_STATUS_ID) from HRSC.HR_REQUEST_STATUS HRS  where HRS.HR_REQUEST_ID = REQ.HR_REQUEST_ID) AND
					...
					AND (CHARINDEX(BC.BUSINESS_CENTER_CODE, @BCList) > 0 OR (REQ.HR_REQUEST_TYPE_ID = @SEPARATION_CLEARANCE AND @showSC = 1) OR @showHRSC = 1 OR (P.POSITION_ID IS NOT NULL AND @showPosition = 1))
