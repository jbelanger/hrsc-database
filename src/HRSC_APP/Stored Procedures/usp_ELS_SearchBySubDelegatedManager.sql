...
-- =============================================
CREATE   PROCEDURE  [HRSC_APP].[usp_ELS_SearchBySubDelegatedManager]
...
BEGIN
	...
	(SELECT  ID
	 FROM [HRSC].[ELS_Request_TEMP]
	  WHERE  upper(delegatedmanager) LIKE upper('%' + @pSubDelegatedManager + '%') 
        
	) as NewReq

	SELECT * FROM
	(SELECT 
		 Row_Number() over (order by delegatedmanager, ID ASC) as RowIndex
		,[id]	AS ID
