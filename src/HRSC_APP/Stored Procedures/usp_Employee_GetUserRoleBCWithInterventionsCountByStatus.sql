  ...
  (SELECT UR.EMPLOYEE_ID,
   ...
   (SELECT URBC.EMPLOYEE_ID
	   ...
	   WHERE URBC.BUSINESS_CENTER_ID = @pBCID AND 
		  ...
		  URBC.EXPIRY_DATE IS NULL AND BC.EXPIRY_DATE IS NULL AND E.EXPIRY_DATE IS NULL) AS UR
   LEFT OUTER JOIN [HRSC].[HR_RQST_INTRVNT_HRSC_USER] IER ON UR.EMPLOYEE_ID = IER.EMPLOYEE_ID
