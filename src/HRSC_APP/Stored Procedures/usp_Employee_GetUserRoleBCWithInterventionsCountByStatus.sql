...
AS
  SELECT E.EMPLOYEE_ID AS Id, E.EMPLOYEE_EMAIL_ADDRESS AS EmailAddress, E.EMPLOYEE_GIVEN_NAME AS GivenName, E.EMPLOYEE_SURNAME AS Surname, 
         E.WORK_PHONE_NUMBER AS WorkPhoneNumber, L.LANGUAGE_CODE2 AS LanguageId,
		 ET.TotalLeadActives, ET.TotalCollActives, ET.TotalLeadPending, ET.TotalCollPending
  FROM
  (SELECT IER.EMPLOYEE_ID,
   ...
   FROM 
   [HRSC].[HR_RQST_INTRVNT_HRSC_USER] IER
   INNER JOIN [HRSC].[HR_REQUEST_INTERVENTION] RI ON IER.REQUEST_INTERVENTION_ID = RI.HR_REQUEST_INTERVENTION_ID
   INNER JOIN [HRSC].[CD_EMPLOYEE_ROLE] ER ON IER.EMPLOYEE_ROLE_ID = ER.EMPLOYEE_ROLE_ID
   INNER JOIN [HRSC].[CD_REQUEST_STATUS] RS ON RI.REQUEST_STATUS_ID = RS.REQUEST_STATUS_ID
   WHERE IER.EMPLOYEE_ID IN 
	  (SELECT URBC.EMPLOYEE_ID
	   ...
	   WHERE URBC.BUSINESS_CENTER_ID = @pBCID AND 
		  ...
		  URBC.EXPIRY_DATE IS NULL AND BC.EXPIRY_DATE IS NULL AND E.EXPIRY_DATE IS NULL)
   GROUP BY IER.[EMPLOYEE_ID]) AS ET
