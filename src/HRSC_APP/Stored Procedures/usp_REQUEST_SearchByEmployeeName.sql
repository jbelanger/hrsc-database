...
CREATE   PROCEDURE [HRSC_APP].[usp_REQUEST_SearchByEmployeeName]
@RowIndexStart bigint, @RowIndexEnd bigint, @pGivenName varchar(30), @pSurName varchar(30), @recCount bigint OUTPUT
WITH EXEC AS CALLER
AS
------------------------------------------------------
-- usp_REQUEST_SearchByEmployeeName]
-- 
-- Tony Paradis
------------------------------------------------------
...
FROM	(SELECT		REQ.HR_REQUEST_ID
		 FROM		HRSC.HR_REQUEST REQ
					...
					INNER JOIN HRSC.SEPARATION_CLEARANCE SC ON SC.HR_REQUEST_ID = REQ.HR_REQUEST_ID
		 ...
		 WHERE		STAT.HR_REQUEST_STATUS_ID = (SELECT MAX(HR_REQUEST_STATUS_ID) FROM HRSC.HR_REQUEST_STATUS HRS WHERE HRS.HR_REQUEST_ID = REQ.HR_REQUEST_ID) AND
					...
					UPPER(RC.GIVEN_NAME) = UPPER(@pGivenName)
