...
-- =============================================
CREATE   PROCEDURE [HRSC_APP].[usp_ASRF_SearchBySubDelegatedManager]
	@pSubDelegatedManager nvarchar(50),
...
BEGIN
	...
	(SELECT  F_ID
	 FROM HRSC.[ASRF_FORMS]
	  WHERE  upper(F_SubDelegatedManager) LIKE upper('%' + @pSubDelegatedManager + '%') AND 
...
SELECT * FROM
(SELECT 
	 Row_Number() over (order by F_SubDelegatedManager, F_ID asc) as RowIndex
	  , [F_ID]
