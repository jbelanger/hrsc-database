...
CREATE   PROCEDURE [HRSC_APP].[USP_WorkbenchMainBC]
@BC_ID bigint
WITH EXEC AS CALLER
AS

DECLARE @BC_NAME_EN NVARCHAR(250)
DECLARE @BC_NAME_FR NVARCHAR(250)
DECLARE @BC_CODE NVARCHAR(5)
...
SELECT A.BCID AS BC_ID, @BC_NAME_EN AS BC_NAME_EN, @BC_NAME_FR AS BC_NAME_FR, @BC_CODE AS BC_CODE, D.employee_name AS EMPLOYEE_NAME, A.EMPLOYEE_ID AS EMPLOYEE_ID,
...
FROM
	...
	(SELECT DISTINCT Rl.EMPLOYEE_ID, @BC_ID AS BCID
	...
	WHERE I.BUSINESS_CENTER_ID = @BC_ID
	...
	WHERE I.BUSINESS_CENTER_ID = @BC_ID AND SS.REQUEST_STATUS_CODE IN ('ATSF', 'NEW', 'NUPD','TSF')
	GROUP BY I.BUSINESS_CENTER_ID) 
	AS B ON A.BCID = B.BUSINESS_CENTER_ID
	...
	JOIN HRSC.CD_REQUEST_STATUS SS ON SS.REQUEST_STATUS_ID = I.REQUEST_STATUS_ID
	WHERE I.BUSINESS_CENTER_ID = @BC_ID
			AND SS.REQUEST_STATUS_CODE IN ('ASGN', 'PROC', 'SIGND', 'RTNFC', 'WAIT', 'WSIGN', 'RTRNC', 'RTURN')  
	GROUP BY EMPLOYEE_ID) 
	AS C ON A.EMPLOYEE_ID = C.EMPLOYEE_ID

	LEFT OUTER JOIN

	(SELECT EMPLOYEE_ID, EMPLOYEE_SURNAME + ', ' + EMPLOYEE_GIVEN_NAME AS employee_name
	FROM HRSC.EMPLOYEE) AS D ON A.EMPLOYEE_ID = D.EMPLOYEE_ID
...
--      SELECT distinct Rl.EMPLOYEE_ID

--      FROM HRSC.HR_RQST_INTRVNT_HRSC_USER Rl  
--	  JOIN HRSC.HR_REQUEST_INTERVENTION I ON Rl.REQUEST_INTERVENTION_ID = I.HR_REQUEST_INTERVENTION_ID
--	  JOIN HRSC.EMPLOYEE E ON Rl.EMPLOYEE_ID = E.EMPLOYEE_ID
--      WHERE 
--      I.BUSINESS_CENTER_ID = @BC_ID
--      and I.REQUEST_STATUS_ID not in (select  REQUEST_STATUS_ID from HRSC.CD_REQUEST_STATUS where REQUEST_STATUS_CODE in (N'CANC', N'CLOSE', N'REJ', N'NOACT'))
--      --ORDER by emp_name
...
--  SELECT * from @BC_INFO order by employee_name;
