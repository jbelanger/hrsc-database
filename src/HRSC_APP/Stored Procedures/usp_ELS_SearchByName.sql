...
-- =============================================
CREATE   PROCEDURE [HRSC_APP].[usp_ELS_SearchByName]
 	...
 	IF @pGivenName <> '' AND @pSurName <> ''
		BEGIN
			SELECT @pRecCount = COUNT (ID) FROM
				...
				WHERE  upper(employeeGivenName) like upper(@pGivenName + '%')  AND
						 upper(employeeSurname) like upper(@pSurName + '%')
	...
	ELSE
		IF @pGivenName <> '' 
			BEGIN
				...
				(SELECT  ID
				 FROM HRSC.[ELS_Request_TEMP]
				WHERE  upper(employeeGivenName) like upper(@pGivenName + '%')
		...
		ELSE
			BEGIN
				SELECT @pRecCount = COUNT (ID) FROM
					(SELECT  ID
					 FROM HRSC.[ELS_Request_TEMP]
					WHERE  upper(employeeSurname) like upper(@pSurName + '%')
...
IF @pGivenName <> '' AND @pSurName <> ''
	BEGIN
		SELECT * FROM
		(SELECT 
			 Row_Number() over (order by DateSubmitted asc, employeeSurname asc, employeeGivenName asc) as RowIndex
			  ,[id]	AS ID
