...
DECLARE @mergeOutput1653580929 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1653580929;

DECLARE @mergeError1653580929 int,
@mergeCount1653580929 int,
@mergeCountIns1653580929 int,
@mergeCountUpd1653580929 int,
@mergeCountDel1653580929 int
SELECT @mergeError1653580929 = @@ERROR
SELECT @mergeCount1653580929 = COUNT(1), @mergeCountIns1653580929 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1653580929 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1653580929 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1653580929
IF @mergeError1653580929 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_HR_GUIDE_TYPE]. Rows affected: ' + CAST(@mergeCount1653580929 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_HR_GUIDE_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1653580929,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1653580929,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1653580929,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1653580929,0) AS VARCHAR(100)) + ')' ;
 END
