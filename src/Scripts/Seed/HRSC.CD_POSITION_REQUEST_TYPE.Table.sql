...
DECLARE @mergeOutput2093250512 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput2093250512;

DECLARE @mergeError2093250512 int,
@mergeCount2093250512 int,
@mergeCountIns2093250512 int,
@mergeCountUpd2093250512 int,
@mergeCountDel2093250512 int
SELECT @mergeError2093250512 = @@ERROR
SELECT @mergeCount2093250512 = COUNT(1), @mergeCountIns2093250512 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd2093250512 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel2093250512 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput2093250512
IF @mergeError2093250512 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_POSITION_REQUEST_TYPE]. Rows affected: ' + CAST(@mergeCount2093250512 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_POSITION_REQUEST_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount2093250512,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns2093250512,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd2093250512,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel2093250512,0) AS VARCHAR(100)) + ')' ;
 END
