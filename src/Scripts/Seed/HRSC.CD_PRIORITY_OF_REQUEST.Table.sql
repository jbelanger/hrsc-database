...
DECLARE @mergeOutput2071678428 TABLE ( [DMLAction] VARCHAR(6) );
MERGE INTO [HRSC].[CD_PRIORITY_OF_REQUEST] AS [Target]
USING (VALUES
  (2,N'High',N'Haute',N'High',N'Haute','2011-12-08T15:55:24.230',NULL,N'Gaétan',N'Gaétan','2011-12-08T15:55:24.230','2011-12-29T10:31:21.477',N'HIGH',1)
 ,(3,N'Normal',N'Normale',N'Normal',N'Normale','2011-12-08T15:55:24.230',NULL,N'Gaétan',N'Gaétan','2011-12-08T15:55:24.230','2011-12-29T10:31:21.477',N'NORM',1)
 ,(4,N'Low',N'Basse',N'Low',N'Basse','2011-12-08T15:55:24.230',NULL,N'Gaétan',N'Gaétan','2011-12-08T15:55:24.230','2011-12-29T10:31:21.477',N'LOW',1)
...
OUTPUT $action INTO @mergeOutput2071678428;

DECLARE @mergeError2071678428 int,
@mergeCount2071678428 int,
@mergeCountIns2071678428 int,
@mergeCountUpd2071678428 int,
@mergeCountDel2071678428 int
SELECT @mergeError2071678428 = @@ERROR
SELECT @mergeCount2071678428 = COUNT(1), @mergeCountIns2071678428 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd2071678428 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel2071678428 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput2071678428
IF @mergeError2071678428 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_PRIORITY_OF_REQUEST]. Rows affected: ' + CAST(@mergeCount2071678428 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_PRIORITY_OF_REQUEST] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount2071678428,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns2071678428,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd2071678428,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel2071678428,0) AS VARCHAR(100)) + ')' ;
 END
