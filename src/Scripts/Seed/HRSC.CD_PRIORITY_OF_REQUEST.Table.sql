...
IF @mergeError162099618 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_PRIORITY_OF_REQUEST]. Rows affected: ' + CAST(@mergeCount162099618 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_PRIORITY_OF_REQUEST] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount162099618,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns162099618,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd162099618,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel162099618,0) AS VARCHAR(100)) + ')' ;
 END
