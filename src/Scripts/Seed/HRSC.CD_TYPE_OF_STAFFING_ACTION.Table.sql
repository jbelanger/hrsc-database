...
DECLARE @mergeOutput1090102924 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1090102924;

DECLARE @mergeError1090102924 int,
@mergeCount1090102924 int,
@mergeCountIns1090102924 int,
@mergeCountUpd1090102924 int,
@mergeCountDel1090102924 int
SELECT @mergeError1090102924 = @@ERROR
SELECT @mergeCount1090102924 = COUNT(1), @mergeCountIns1090102924 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1090102924 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1090102924 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1090102924
IF @mergeError1090102924 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_TYPE_OF_STAFFING_ACTION]. Rows affected: ' + CAST(@mergeCount1090102924 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_TYPE_OF_STAFFING_ACTION] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1090102924,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1090102924,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1090102924,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1090102924,0) AS VARCHAR(100)) + ')' ;
 END
