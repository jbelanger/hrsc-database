...
DECLARE @mergeOutput1255779631 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1255779631;

DECLARE @mergeError1255779631 int,
@mergeCount1255779631 int,
@mergeCountIns1255779631 int,
@mergeCountUpd1255779631 int,
@mergeCountDel1255779631 int
SELECT @mergeError1255779631 = @@ERROR
SELECT @mergeCount1255779631 = COUNT(1), @mergeCountIns1255779631 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1255779631 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1255779631 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1255779631
IF @mergeError1255779631 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_TYPE_OF_STAFFING_ACTION]. Rows affected: ' + CAST(@mergeCount1255779631 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_TYPE_OF_STAFFING_ACTION] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1255779631,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1255779631,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1255779631,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1255779631,0) AS VARCHAR(100)) + ')' ;
 END
