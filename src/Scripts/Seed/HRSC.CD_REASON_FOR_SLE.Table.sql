...
DECLARE @mergeOutput1095779061 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1095779061;

DECLARE @mergeError1095779061 int,
@mergeCount1095779061 int,
@mergeCountIns1095779061 int,
@mergeCountUpd1095779061 int,
@mergeCountDel1095779061 int
SELECT @mergeError1095779061 = @@ERROR
SELECT @mergeCount1095779061 = COUNT(1), @mergeCountIns1095779061 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1095779061 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1095779061 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1095779061
IF @mergeError1095779061 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_REASON_FOR_SLE]. Rows affected: ' + CAST(@mergeCount1095779061 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_REASON_FOR_SLE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1095779061,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1095779061,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1095779061,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1095779061,0) AS VARCHAR(100)) + ')' ;
 END
