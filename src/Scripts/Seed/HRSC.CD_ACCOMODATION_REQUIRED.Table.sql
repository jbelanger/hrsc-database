...
DECLARE @mergeOutput709577566 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput709577566;

DECLARE @mergeError709577566 int,
@mergeCount709577566 int,
@mergeCountIns709577566 int,
@mergeCountUpd709577566 int,
@mergeCountDel709577566 int
SELECT @mergeError709577566 = @@ERROR
SELECT @mergeCount709577566 = COUNT(1), @mergeCountIns709577566 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd709577566 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel709577566 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput709577566
IF @mergeError709577566 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_ACCOMODATION_REQUIRED]. Rows affected: ' + CAST(@mergeCount709577566 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_ACCOMODATION_REQUIRED] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount709577566,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns709577566,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd709577566,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel709577566,0) AS VARCHAR(100)) + ')' ;
 END
