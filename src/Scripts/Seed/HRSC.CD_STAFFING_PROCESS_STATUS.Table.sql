...
DECLARE @mergeOutput818101955 TABLE ( [DMLAction] VARCHAR(6) );
...
USING (VALUES
 ...
 ,(4,'2020-01-17',NULL,N'ISL_INSERT','2020-01-17',NULL,NULL,N'2',N'Completed',N'Complété')
 ,(5,'2020-01-17',NULL,N'ISL_INSERT','2020-01-17',NULL,NULL,N'3',N'Completed - Unproductive',N'Complété - Non productif')
 ,(13,'2020-01-17',NULL,N'ISL_INSERT','2020-01-17',NULL,NULL,N'11',N'Cancelled',N'Annulé')
...
OUTPUT $action INTO @mergeOutput818101955;

DECLARE @mergeError818101955 int,
@mergeCount818101955 int,
@mergeCountIns818101955 int,
@mergeCountUpd818101955 int,
@mergeCountDel818101955 int
SELECT @mergeError818101955 = @@ERROR
SELECT @mergeCount818101955 = COUNT(1), @mergeCountIns818101955 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd818101955 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel818101955 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput818101955
IF @mergeError818101955 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_STAFFING_PROCESS_STATUS]. Rows affected: ' + CAST(@mergeCount818101955 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_STAFFING_PROCESS_STATUS] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount818101955,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns818101955,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd818101955,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel818101955,0) AS VARCHAR(100)) + ')' ;
 END
