...
DECLARE @mergeOutput1079726949 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1079726949;

DECLARE @mergeError1079726949 int,
@mergeCount1079726949 int,
@mergeCountIns1079726949 int,
@mergeCountUpd1079726949 int,
@mergeCountDel1079726949 int
SELECT @mergeError1079726949 = @@ERROR
SELECT @mergeCount1079726949 = COUNT(1), @mergeCountIns1079726949 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1079726949 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1079726949 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1079726949
IF @mergeError1079726949 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_STAFFING_PROCESS_STATUS]. Rows affected: ' + CAST(@mergeCount1079726949 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_STAFFING_PROCESS_STATUS] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1079726949,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1079726949,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1079726949,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1079726949,0) AS VARCHAR(100)) + ')' ;
 END
