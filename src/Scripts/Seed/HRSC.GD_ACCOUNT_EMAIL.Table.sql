...
DECLARE @mergeOutput1593772735 TABLE ( [DMLAction] VARCHAR(6) );
...
USING (VALUES
 ...
 ,(15,N'FINANCE',N'FINANCE',N'EMAIL SENT TO SFT WHEN INTERVENTION TRANSFERT TO HRSB',N'EMAIL SENT TO SFT WHEN INTERVENTION TRANSFERT TO HRSB','2016-04-21T00:00:00',N'EDSC.DGIIT.SFASalaire-FPSSalary.IITB.ESDC@hrsdc-rhdcc.gc.ca',NULL,N'HRSC 4.13','2023-01-27T11:25:24.857',NULL,NULL,8)
 ,(16,N'PAR Email3',N'PAR Email3',N'EMAIL SENT TO NPC FOR ACTION',N'EMAIL ENVOYE A CNP POUR ACTION','2023-06-29T14:57:07.153',N'na-hrsc-csrh-web-app-gd@hrsdc-rhdcc.gc.ca','2023-06-29T14:57:07.153',N'System','2023-06-29T14:57:07.153',NULL,NULL,7)
...
OUTPUT $action INTO @mergeOutput1593772735;

DECLARE @mergeError1593772735 int,
@mergeCount1593772735 int,
@mergeCountIns1593772735 int,
@mergeCountUpd1593772735 int,
@mergeCountDel1593772735 int
SELECT @mergeError1593772735 = @@ERROR
SELECT @mergeCount1593772735 = COUNT(1), @mergeCountIns1593772735 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1593772735 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1593772735 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1593772735
IF @mergeError1593772735 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[GD_ACCOUNT_EMAIL]. Rows affected: ' + CAST(@mergeCount1593772735 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[GD_ACCOUNT_EMAIL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1593772735,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1593772735,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1593772735,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1593772735,0) AS VARCHAR(100)) + ')' ;
 END
