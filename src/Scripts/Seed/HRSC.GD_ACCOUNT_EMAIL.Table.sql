...
DECLARE @mergeOutput1986106116 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1986106116;

DECLARE @mergeError1986106116 int,
@mergeCount1986106116 int,
@mergeCountIns1986106116 int,
@mergeCountUpd1986106116 int,
@mergeCountDel1986106116 int
SELECT @mergeError1986106116 = @@ERROR
SELECT @mergeCount1986106116 = COUNT(1), @mergeCountIns1986106116 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1986106116 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1986106116 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1986106116
IF @mergeError1986106116 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[GD_ACCOUNT_EMAIL]. Rows affected: ' + CAST(@mergeCount1986106116 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[GD_ACCOUNT_EMAIL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1986106116,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1986106116,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1986106116,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1986106116,0) AS VARCHAR(100)) + ')' ;
 END
