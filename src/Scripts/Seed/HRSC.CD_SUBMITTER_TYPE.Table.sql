...
DECLARE @mergeOutput191391801 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput191391801;

DECLARE @mergeError191391801 int,
@mergeCount191391801 int,
@mergeCountIns191391801 int,
@mergeCountUpd191391801 int,
@mergeCountDel191391801 int
SELECT @mergeError191391801 = @@ERROR
SELECT @mergeCount191391801 = COUNT(1), @mergeCountIns191391801 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd191391801 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel191391801 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput191391801
IF @mergeError191391801 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_SUBMITTER_TYPE]. Rows affected: ' + CAST(@mergeCount191391801 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_SUBMITTER_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount191391801,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns191391801,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd191391801,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel191391801,0) AS VARCHAR(100)) + ')' ;
 END
