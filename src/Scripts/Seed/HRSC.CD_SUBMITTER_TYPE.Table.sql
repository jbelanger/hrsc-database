...
DECLARE @mergeOutput1122103038 TABLE ( [DMLAction] VARCHAR(6) );
MERGE INTO [HRSC].[CD_SUBMITTER_TYPE] AS [Target]
USING (VALUES
  (1,N'EMP',N'Employee',N'Employ?',NULL,NULL,NULL,N'HRSC5.0',NULL,'2021-10-30T05:25:26.987',NULL)
...
OUTPUT $action INTO @mergeOutput1122103038;

DECLARE @mergeError1122103038 int,
@mergeCount1122103038 int,
@mergeCountIns1122103038 int,
@mergeCountUpd1122103038 int,
@mergeCountDel1122103038 int
SELECT @mergeError1122103038 = @@ERROR
SELECT @mergeCount1122103038 = COUNT(1), @mergeCountIns1122103038 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1122103038 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1122103038 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1122103038
IF @mergeError1122103038 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_SUBMITTER_TYPE]. Rows affected: ' + CAST(@mergeCount1122103038 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_SUBMITTER_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1122103038,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1122103038,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1122103038,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1122103038,0) AS VARCHAR(100)) + ')' ;
 END
