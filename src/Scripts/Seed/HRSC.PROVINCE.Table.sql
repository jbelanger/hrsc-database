...
DECLARE @mergeOutput1794105432 TABLE ( [DMLAction] VARCHAR(6) );
...
USING (VALUES
 ...
 ,(36,N'NS',12,'2001-06-21T15:00:31',NULL,N'Nova Scotia',N'Nouvelle-?cosse',3,N'IRM_ADMIN',N'IRM_ADMIN','2002-05-10T11:36:39','2003-08-14T11:17:12')
 ,(37,N'PE',11,'2001-06-21T15:00:31',NULL,N'Prince Edward Island',N'?le-du-Prince-?douard',4,N'IRM_ADMIN',N'IRM_ADMIN','2002-05-10T11:36:39','2003-08-14T11:17:15')
 ...
 ,(39,N'QC',24,'2001-06-21T15:00:31',NULL,N'Quebec',N'Qu?bec',6,N'IRM_ADMIN',N'IRM_ADMIN','2002-05-10T11:36:39','2003-08-14T11:17:21')
...
OUTPUT $action INTO @mergeOutput1794105432;

DECLARE @mergeError1794105432 int,
@mergeCount1794105432 int,
@mergeCountIns1794105432 int,
@mergeCountUpd1794105432 int,
@mergeCountDel1794105432 int
SELECT @mergeError1794105432 = @@ERROR
SELECT @mergeCount1794105432 = COUNT(1), @mergeCountIns1794105432 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1794105432 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1794105432 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1794105432
IF @mergeError1794105432 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[PROVINCE]. Rows affected: ' + CAST(@mergeCount1794105432 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[PROVINCE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1794105432,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1794105432,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1794105432,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1794105432,0) AS VARCHAR(100)) + ')' ;
 END
