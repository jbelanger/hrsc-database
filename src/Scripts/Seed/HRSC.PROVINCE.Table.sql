...
DECLARE @mergeOutput1682105033 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1682105033;

DECLARE @mergeError1682105033 int,
@mergeCount1682105033 int,
@mergeCountIns1682105033 int,
@mergeCountUpd1682105033 int,
@mergeCountDel1682105033 int
SELECT @mergeError1682105033 = @@ERROR
SELECT @mergeCount1682105033 = COUNT(1), @mergeCountIns1682105033 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1682105033 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1682105033 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1682105033
IF @mergeError1682105033 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[PROVINCE]. Rows affected: ' + CAST(@mergeCount1682105033 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[PROVINCE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1682105033,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1682105033,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1682105033,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1682105033,0) AS VARCHAR(100)) + ')' ;
 END
