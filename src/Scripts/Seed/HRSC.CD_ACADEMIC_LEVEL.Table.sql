...
OUTPUT $action INTO @mergeOutput1487344363;

DECLARE @mergeError1487344363 int,
@mergeCount1487344363 int,
@mergeCountIns1487344363 int,
@mergeCountUpd1487344363 int,
@mergeCountDel1487344363 int
SELECT @mergeError1487344363 = @@ERROR
SELECT @mergeCount1487344363 = COUNT(1), @mergeCountIns1487344363 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1487344363 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1487344363 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1487344363
IF @mergeError1487344363 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_ACADEMIC_LEVEL]. Rows affected: ' + CAST(@mergeCount1487344363 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_ACADEMIC_LEVEL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1487344363,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1487344363,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1487344363,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1487344363,0) AS VARCHAR(100)) + ')' ;
 END
