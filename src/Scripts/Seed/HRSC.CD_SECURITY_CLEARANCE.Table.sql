...
DECLARE @mergeOutput73767320 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput73767320;

DECLARE @mergeError73767320 int,
@mergeCount73767320 int,
@mergeCountIns73767320 int,
@mergeCountUpd73767320 int,
@mergeCountDel73767320 int
SELECT @mergeError73767320 = @@ERROR
SELECT @mergeCount73767320 = COUNT(1), @mergeCountIns73767320 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd73767320 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel73767320 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput73767320
IF @mergeError73767320 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_SECURITY_CLEARANCE]. Rows affected: ' + CAST(@mergeCount73767320 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_SECURITY_CLEARANCE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount73767320,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns73767320,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd73767320,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel73767320,0) AS VARCHAR(100)) + ')' ;
 END
