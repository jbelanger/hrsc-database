...
OUTPUT $action INTO @mergeOutput1127675065;

DECLARE @mergeError1127675065 int,
@mergeCount1127675065 int,
@mergeCountIns1127675065 int,
@mergeCountUpd1127675065 int,
@mergeCountDel1127675065 int
SELECT @mergeError1127675065 = @@ERROR
SELECT @mergeCount1127675065 = COUNT(1), @mergeCountIns1127675065 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1127675065 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1127675065 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1127675065
IF @mergeError1127675065 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_DOCUMENT_TYPE]. Rows affected: ' + CAST(@mergeCount1127675065 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_DOCUMENT_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1127675065,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1127675065,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1127675065,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1127675065,0) AS VARCHAR(100)) + ')' ;
 END
