...
DECLARE @mergeOutput1853249657 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1853249657;

DECLARE @mergeError1853249657 int,
@mergeCount1853249657 int,
@mergeCountIns1853249657 int,
@mergeCountUpd1853249657 int,
@mergeCountDel1853249657 int
SELECT @mergeError1853249657 = @@ERROR
SELECT @mergeCount1853249657 = COUNT(1), @mergeCountIns1853249657 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1853249657 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1853249657 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1853249657
IF @mergeError1853249657 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_FISCAL_YEAR]. Rows affected: ' + CAST(@mergeCount1853249657 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_FISCAL_YEAR] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1853249657,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1853249657,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1853249657,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1853249657,0) AS VARCHAR(100)) + ')' ;
 END
