...
DECLARE @mergeOutput1118627028 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1118627028;

DECLARE @mergeError1118627028 int,
@mergeCount1118627028 int,
@mergeCountIns1118627028 int,
@mergeCountUpd1118627028 int,
@mergeCountDel1118627028 int
SELECT @mergeError1118627028 = @@ERROR
SELECT @mergeCount1118627028 = COUNT(1), @mergeCountIns1118627028 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1118627028 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1118627028 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1118627028
IF @mergeError1118627028 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_FISCAL_YEAR]. Rows affected: ' + CAST(@mergeCount1118627028 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_FISCAL_YEAR] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1118627028,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1118627028,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1118627028,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1118627028,0) AS VARCHAR(100)) + ')' ;
 END
