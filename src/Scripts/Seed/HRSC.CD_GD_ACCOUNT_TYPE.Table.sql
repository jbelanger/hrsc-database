...
DECLARE @mergeOutput1605580758 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1605580758;

DECLARE @mergeError1605580758 int,
@mergeCount1605580758 int,
@mergeCountIns1605580758 int,
@mergeCountUpd1605580758 int,
@mergeCountDel1605580758 int
SELECT @mergeError1605580758 = @@ERROR
SELECT @mergeCount1605580758 = COUNT(1), @mergeCountIns1605580758 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1605580758 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1605580758 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1605580758
IF @mergeError1605580758 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_GD_ACCOUNT_TYPE]. Rows affected: ' + CAST(@mergeCount1605580758 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_GD_ACCOUNT_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1605580758,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1605580758,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1605580758,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1605580758,0) AS VARCHAR(100)) + ')' ;
 END
