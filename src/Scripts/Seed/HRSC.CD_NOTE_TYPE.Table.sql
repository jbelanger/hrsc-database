...
DECLARE @mergeOutput951778548 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput951778548;

DECLARE @mergeError951778548 int,
@mergeCount951778548 int,
@mergeCountIns951778548 int,
@mergeCountUpd951778548 int,
@mergeCountDel951778548 int
SELECT @mergeError951778548 = @@ERROR
SELECT @mergeCount951778548 = COUNT(1), @mergeCountIns951778548 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd951778548 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel951778548 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput951778548
IF @mergeError951778548 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_NOTE_TYPE]. Rows affected: ' + CAST(@mergeCount951778548 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_NOTE_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount951778548,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns951778548,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd951778548,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel951778548,0) AS VARCHAR(100)) + ')' ;
 END
