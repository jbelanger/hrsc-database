...
DECLARE @mergeOutput1909581841 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1909581841;

DECLARE @mergeError1909581841 int,
@mergeCount1909581841 int,
@mergeCountIns1909581841 int,
@mergeCountUpd1909581841 int,
@mergeCountDel1909581841 int
SELECT @mergeError1909581841 = @@ERROR
SELECT @mergeCount1909581841 = COUNT(1), @mergeCountIns1909581841 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1909581841 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1909581841 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1909581841
IF @mergeError1909581841 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_LANGUAGE]. Rows affected: ' + CAST(@mergeCount1909581841 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_LANGUAGE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1909581841,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1909581841,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1909581841,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1909581841,0) AS VARCHAR(100)) + ')' ;
 END
