...
DECLARE @mergeOutput2062630391 TABLE ( [DMLAction] VARCHAR(6) );
...
USING (VALUES
 ...
 ,(5,2,N'French',N'Fran?ais',N'FR',N'FR',N'French',N'Fran?ais','2011-12-08T12:02:14.773',NULL,N'Gaetan',NULL,'2011-12-08T12:02:14.773',NULL,N'0')
 ...
 ,(8,4,N'No Preference',N'Aucune Pr?f?rence',N'NP',N'NP',N'No Preference',N'Aucune Pr?f?rence','2012-06-01T13:18:34.407',NULL,N'eric.nolet',NULL,'2012-06-27T13:26:56.637',NULL,N'0')
...
WHEN MATCHED AND (
	NULLIF([Source].[LANGUAGE_CODE], [Target].[LANGUAGE_CODE]) IS NOT NULL OR NULLIF([Target].[LANGUAGE_CODE], [Source].[LANGUAGE_CODE]) IS NOT NULL OR 
...
WHEN NOT MATCHED BY TARGET THEN
 INSERT([LANGUAGE_ID],[LANGUAGE_CODE],[LANGUAGE_NAME_EN],[LANGUAGE_NAME_FR],[LANGUAGE_ABRVTD_NAME_EN],[LANGUAGE_ABRVTD_NAME_FR],[LANGUAGE_DESC_EN],[LANGUAGE_DESC_FR],[EFFECTIVE_DATE],[EXPIRY_DATE],[USER_CREATED],[USER_UPDATED],[DATE_CREATED],[DATE_UPDATED],[LANGUAGE_CODE2])
 VALUES([Source].[LANGUAGE_ID],[Source].[LANGUAGE_CODE],[Source].[LANGUAGE_NAME_EN],[Source].[LANGUAGE_NAME_FR],[Source].[LANGUAGE_ABRVTD_NAME_EN],[Source].[LANGUAGE_ABRVTD_NAME_FR],[Source].[LANGUAGE_DESC_EN],[Source].[LANGUAGE_DESC_FR],[Source].[EFFECTIVE_DATE],[Source].[EXPIRY_DATE],[Source].[USER_CREATED],[Source].[USER_UPDATED],[Source].[DATE_CREATED],[Source].[DATE_UPDATED],[Source].[LANGUAGE_CODE2])
OUTPUT $action INTO @mergeOutput2062630391;

DECLARE @mergeError2062630391 int,
@mergeCount2062630391 int,
@mergeCountIns2062630391 int,
@mergeCountUpd2062630391 int,
@mergeCountDel2062630391 int
SELECT @mergeError2062630391 = @@ERROR
SELECT @mergeCount2062630391 = COUNT(1), @mergeCountIns2062630391 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd2062630391 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel2062630391 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput2062630391
IF @mergeError2062630391 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_LANGUAGE]. Rows affected: ' + CAST(@mergeCount2062630391 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_LANGUAGE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount2062630391,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns2062630391,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd2062630391,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel2062630391,0) AS VARCHAR(100)) + ')' ;
 END
