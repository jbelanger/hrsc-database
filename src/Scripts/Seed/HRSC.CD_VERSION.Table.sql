...
DECLARE @mergeOutput1762105318 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1762105318;

DECLARE @mergeError1762105318 int,
@mergeCount1762105318 int,
@mergeCountIns1762105318 int,
@mergeCountUpd1762105318 int,
@mergeCountDel1762105318 int
SELECT @mergeError1762105318 = @@ERROR
SELECT @mergeCount1762105318 = COUNT(1), @mergeCountIns1762105318 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1762105318 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1762105318 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1762105318
IF @mergeError1762105318 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_VERSION]. Rows affected: ' + CAST(@mergeCount1762105318 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_VERSION] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1762105318,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1762105318,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1762105318,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1762105318,0) AS VARCHAR(100)) + ')' ;
 END
