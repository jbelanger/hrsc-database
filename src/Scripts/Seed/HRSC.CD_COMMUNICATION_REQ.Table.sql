...
DECLARE @mergeOutput1843537651 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1843537651;

DECLARE @mergeError1843537651 int,
@mergeCount1843537651 int,
@mergeCountIns1843537651 int,
@mergeCountUpd1843537651 int,
@mergeCountDel1843537651 int
SELECT @mergeError1843537651 = @@ERROR
SELECT @mergeCount1843537651 = COUNT(1), @mergeCountIns1843537651 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1843537651 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1843537651 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1843537651
IF @mergeError1843537651 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_COMMUNICATION_REQ]. Rows affected: ' + CAST(@mergeCount1843537651 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_COMMUNICATION_REQ] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1843537651,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1843537651,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1843537651,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1843537651,0) AS VARCHAR(100)) + ')' ;
 END
