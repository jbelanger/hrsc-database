...
IF @mergeError542624976 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_COMMUNICATION_REQ]. Rows affected: ' + CAST(@mergeCount542624976 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_COMMUNICATION_REQ] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount542624976,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns542624976,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd542624976,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel542624976,0) AS VARCHAR(100)) + ')' ;
 END
