...
DECLARE @mergeOutput2045250341 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput2045250341;

DECLARE @mergeError2045250341 int,
@mergeCount2045250341 int,
@mergeCountIns2045250341 int,
@mergeCountUpd2045250341 int,
@mergeCountDel2045250341 int
SELECT @mergeError2045250341 = @@ERROR
SELECT @mergeCount2045250341 = COUNT(1), @mergeCountIns2045250341 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd2045250341 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel2045250341 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput2045250341
IF @mergeError2045250341 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_LANGUAGE_LEVEL]. Rows affected: ' + CAST(@mergeCount2045250341 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_LANGUAGE_LEVEL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount2045250341,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns2045250341,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd2045250341,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel2045250341,0) AS VARCHAR(100)) + ')' ;
 END
