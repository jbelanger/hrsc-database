...
DECLARE @mergeOutput226099846 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput226099846;

DECLARE @mergeError226099846 int,
@mergeCount226099846 int,
@mergeCountIns226099846 int,
@mergeCountUpd226099846 int,
@mergeCountDel226099846 int
SELECT @mergeError226099846 = @@ERROR
SELECT @mergeCount226099846 = COUNT(1), @mergeCountIns226099846 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd226099846 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel226099846 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput226099846
IF @mergeError226099846 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_PRIORITY_TYPE]. Rows affected: ' + CAST(@mergeCount226099846 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_PRIORITY_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount226099846,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns226099846,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd226099846,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel226099846,0) AS VARCHAR(100)) + ')' ;
 END
