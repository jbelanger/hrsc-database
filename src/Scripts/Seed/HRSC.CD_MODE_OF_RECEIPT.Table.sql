...
DECLARE @mergeOutput2069582411 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput2069582411;

DECLARE @mergeError2069582411 int,
@mergeCount2069582411 int,
@mergeCountIns2069582411 int,
@mergeCountUpd2069582411 int,
@mergeCountDel2069582411 int
SELECT @mergeError2069582411 = @@ERROR
SELECT @mergeCount2069582411 = COUNT(1), @mergeCountIns2069582411 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd2069582411 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel2069582411 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput2069582411
IF @mergeError2069582411 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_MODE_OF_RECEIPT]. Rows affected: ' + CAST(@mergeCount2069582411 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_MODE_OF_RECEIPT] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount2069582411,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns2069582411,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd2069582411,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel2069582411,0) AS VARCHAR(100)) + ')' ;
 END
