...
OUTPUT $action INTO @mergeOutput1847677630;

DECLARE @mergeError1847677630 int,
@mergeCount1847677630 int,
@mergeCountIns1847677630 int,
@mergeCountUpd1847677630 int,
@mergeCountDel1847677630 int
SELECT @mergeError1847677630 = @@ERROR
SELECT @mergeCount1847677630 = COUNT(1), @mergeCountIns1847677630 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1847677630 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1847677630 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1847677630
IF @mergeError1847677630 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_MODE_OF_RECEIPT]. Rows affected: ' + CAST(@mergeCount1847677630 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_MODE_OF_RECEIPT] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1847677630,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1847677630,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1847677630,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1847677630,0) AS VARCHAR(100)) + ')' ;
 END
