...
DECLARE @mergeOutput1191779403 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1191779403;

DECLARE @mergeError1191779403 int,
@mergeCount1191779403 int,
@mergeCountIns1191779403 int,
@mergeCountUpd1191779403 int,
@mergeCountDel1191779403 int
SELECT @mergeError1191779403 = @@ERROR
SELECT @mergeCount1191779403 = COUNT(1), @mergeCountIns1191779403 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1191779403 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1191779403 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1191779403
IF @mergeError1191779403 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_REPORT_REQUEST_TYPE]. Rows affected: ' + CAST(@mergeCount1191779403 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_REPORT_REQUEST_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1191779403,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1191779403,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1191779403,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1191779403,0) AS VARCHAR(100)) + ')' ;
 END
