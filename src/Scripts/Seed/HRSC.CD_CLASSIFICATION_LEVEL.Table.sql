...
DECLARE @mergeOutput1613248802 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1613248802;

DECLARE @mergeError1613248802 int,
@mergeCount1613248802 int,
@mergeCountIns1613248802 int,
@mergeCountUpd1613248802 int,
@mergeCountDel1613248802 int
SELECT @mergeError1613248802 = @@ERROR
SELECT @mergeCount1613248802 = COUNT(1), @mergeCountIns1613248802 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1613248802 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1613248802 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1613248802
IF @mergeError1613248802 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_CLASSIFICATION_LEVEL]. Rows affected: ' + CAST(@mergeCount1613248802 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_CLASSIFICATION_LEVEL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1613248802,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1613248802,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1613248802,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1613248802,0) AS VARCHAR(100)) + ')' ;
 END
