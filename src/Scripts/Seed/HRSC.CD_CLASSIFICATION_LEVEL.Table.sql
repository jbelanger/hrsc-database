...
DECLARE @mergeOutput1013578649 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1013578649;

DECLARE @mergeError1013578649 int,
@mergeCount1013578649 int,
@mergeCountIns1013578649 int,
@mergeCountUpd1013578649 int,
@mergeCountDel1013578649 int
SELECT @mergeError1013578649 = @@ERROR
SELECT @mergeCount1013578649 = COUNT(1), @mergeCountIns1013578649 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1013578649 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1013578649 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1013578649
IF @mergeError1013578649 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_CLASSIFICATION_LEVEL]. Rows affected: ' + CAST(@mergeCount1013578649 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_CLASSIFICATION_LEVEL] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1013578649,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1013578649,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1013578649,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1013578649,0) AS VARCHAR(100)) + ')' ;
 END
