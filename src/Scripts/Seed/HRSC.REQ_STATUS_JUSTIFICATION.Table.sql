...
DECLARE @mergeOutput1934629935 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput1934629935;

DECLARE @mergeError1934629935 int,
@mergeCount1934629935 int,
@mergeCountIns1934629935 int,
@mergeCountUpd1934629935 int,
@mergeCountDel1934629935 int
SELECT @mergeError1934629935 = @@ERROR
SELECT @mergeCount1934629935 = COUNT(1), @mergeCountIns1934629935 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1934629935 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1934629935 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1934629935
IF @mergeError1934629935 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[REQ_STATUS_JUSTIFICATION]. Rows affected: ' + CAST(@mergeCount1934629935 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[REQ_STATUS_JUSTIFICATION] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1934629935,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1934629935,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1934629935,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1934629935,0) AS VARCHAR(100)) + ')' ;
 END
