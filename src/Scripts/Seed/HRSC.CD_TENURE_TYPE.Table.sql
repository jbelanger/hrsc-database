...
DECLARE @mergeOutput1026102696 TABLE ( [DMLAction] VARCHAR(6) );
...
WHEN NOT MATCHED BY SOURCE THEN 
 DELETE
OUTPUT $action INTO @mergeOutput1026102696;

DECLARE @mergeError1026102696 int,
@mergeCount1026102696 int,
@mergeCountIns1026102696 int,
@mergeCountUpd1026102696 int,
@mergeCountDel1026102696 int
SELECT @mergeError1026102696 = @@ERROR
SELECT @mergeCount1026102696 = COUNT(1), @mergeCountIns1026102696 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd1026102696 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel1026102696 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput1026102696
IF @mergeError1026102696 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_TENURE_TYPE]. Rows affected: ' + CAST(@mergeCount1026102696 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_TENURE_TYPE] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount1026102696,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns1026102696,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd1026102696,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel1026102696,0) AS VARCHAR(100)) + ')' ;
 END
