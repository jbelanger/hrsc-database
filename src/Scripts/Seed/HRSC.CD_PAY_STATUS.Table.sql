...
DECLARE @mergeOutput999778719 TABLE ( [DMLAction] VARCHAR(6) );
...
OUTPUT $action INTO @mergeOutput999778719;

DECLARE @mergeError999778719 int,
@mergeCount999778719 int,
@mergeCountIns999778719 int,
@mergeCountUpd999778719 int,
@mergeCountDel999778719 int
SELECT @mergeError999778719 = @@ERROR
SELECT @mergeCount999778719 = COUNT(1), @mergeCountIns999778719 = SUM(IIF([DMLAction] = 'INSERT', 1, 0)), @mergeCountUpd999778719 = SUM(IIF([DMLAction] = 'UPDATE', 1, 0)), @mergeCountDel999778719 = SUM (IIF([DMLAction] = 'DELETE', 1, 0)) FROM @mergeOutput999778719
IF @mergeError999778719 != 0
 BEGIN
 PRINT 'ERROR OCCURRED IN MERGE FOR [HRSC].[CD_PAY_STATUS]. Rows affected: ' + CAST(@mergeCount999778719 AS VARCHAR(100)); -- SQL should always return zero rows affected
 END
ELSE
 BEGIN
 PRINT '[HRSC].[CD_PAY_STATUS] rows affected by MERGE: ' + CAST(COALESCE(@mergeCount999778719,0) AS VARCHAR(100)) + ' (Inserted: ' + CAST(COALESCE(@mergeCountIns999778719,0) AS VARCHAR(100)) + '; Updated: ' + CAST(COALESCE(@mergeCountUpd999778719,0) AS VARCHAR(100)) + '; Deleted: ' + CAST(COALESCE(@mergeCountDel999778719,0) AS VARCHAR(100)) + ')' ;
 END
